<div>
    <div class="container-fluid flex-grow-1 px-3 px-md-4 py-3 py-md-4">
        ...
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-body d-flex flex-column flex-md-row justify-content-between align-items-start gap-3">
                ...
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('admin.credentials.create') }}?practice={{ $practice->id }}" class="btn btn-outline-primary">
        ...
        <ul class="nav nav-tabs mb-4 flex-nowrap overflow-auto">
            ...
            <li class="nav-item">
                <button type="button" class="nav-link {{ $activeTab === 'credentialing' ? 'active' : '' }}" wire:click="setTab('credentialing')">
                    Credentialing
            ...
            <li class="nav-item">
                <button type="button" class="nav-link {{ $activeTab === 'locations' ? 'active' : '' }}" wire:click="setTab('locations')">Locations & Contacts</button>
            </li>
            <li class="nav-item">
                <button type="button" class="nav-link {{ $activeTab === 'providers' ? 'active' : '' }}" wire:click="setTab('providers')">
                    Providers
            ...
            <li class="nav-item">
                <button type="button" class="nav-link {{ $activeTab === 'documents' ? 'active' : '' }}" wire:click="setTab('documents')">Documents</button>
            </li>
        </ul>
                        ...
                        <div class="card-body">
                            <div class="row g-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    @if ($website)
                                        <a href="{{ $website }}" target="_blank" rel="noopener" class="fw-semibold text-break">{{ $practice->website }}</a>
                                    @else
                                ...
                                @if ($practice->document_path)
                                    <div class="col-md-4">
                                        ...
                                        <a href="{{ asset('storage/' . $practice->document_path) }}" target="_blank" class="fw-semibold">
                                            <i class="ti tabler-file me-1"></i>{{ $practice->document_original_name ?: 'View file' }}
                                        </a>
                                    </div>
                                @endif
                            </div>
                    ...
                    @if ($practice->addresses->isNotEmpty())
                        <div class="card shadow-sm border-0">
                            <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center">
                                ...
                                <button type="button" class="btn btn-sm btn-link p-0" wire:click="setTab('locations')">View all</button>
                            </div>
                            <div class="card-body">
                                @php $primaryAddress = $practice->addresses->firstWhere('type', 'primary') ?? $practice->addresses->first(); @endphp
                                <div class="fw-semibold mb-2">{{ $addressLabels[$primaryAddress->type] ?? 'Address' }}</div>
        ...
        @if ($activeTab === 'credentialing')
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2">
                    ...
                    <a href="{{ route('admin.credentials.create') }}?practice={{ $practice->id }}" class="btn btn-sm btn-primary">
                        <i class="ti tabler-plus me-1"></i> New Application
                    </a>
                </div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($practice->credentialingCases as $case)
                                <tr>
                                    <td><a href="{{ route('admin.credentials') }}" class="fw-semibold">{{ $case->case_number }}</a></td>
                            ...
                            @forelse($practice->addresses as $address)
                                <div class="border rounded p-3 mb-3 {{ $loop->last ? 'mb-0' : '' }}">
                                    <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                                        <span class="fw-semibold">{{ $addressLabels[$address->type] ?? ($address->location_name ?: 'Address') }}</span>
                                        @if ($address->type === 'primary')
                                            <span class="badge bg-label-{{ $address->status === 'active' ? 'success' : 'secondary' }} text-uppercase">{{ strtoupper($address->status) }}</span>
                                        @endif
                                    </div>
                                    ...
                                    @endif
                                    <p class="mb-2">{{ $address->city }}, {{ $address->state }} {{ $address->zip_code }}@if($address->county) · {{ $address->county }} County @endif</p>
                                    <small class="text-muted d-block">Phone: {{ $address->phone ?: 'N/A' }} · Fax: {{ $address->fax ?: 'N/A' }}</small>
                                </div>
                            @empty
                        ...
                        <div class="card-body">
                            @forelse($practice->locations as $location)
                                <div class="border rounded p-3 mb-3 {{ $loop->last ? 'mb-0' : '' }}">
                                    ...
                                    <p class="mb-1">{{ $location->address1 }}, {{ $location->city }}, {{ $location->state }} {{ $location->zip_code }}</p>
                                    @if ($location->npi)
        ...
        @if ($activeTab === 'providers')
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2">
                    ...
                    <a href="{{ route('admin.provider-practices') }}" class="btn btn-sm btn-outline-primary">
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($practice->providers as $provider)
                                <tr>
                                    <td>
                                        <a href="{{ route('admin.providers.show', $provider->id) }}" class="fw-semibold">{{ $provider->user->name ?? 'N/A' }}</a>
                                    ...
                                    <td><small>{{ $provider->pivot->start_date ? \Illuminate\Support\Carbon::parse($provider->pivot->start_date)->format('m/d/Y') : 'N/A' }}</small></td>
                                    <td><small>{{ $provider->pivot->end_date ? \Illuminate\Support\Carbon::parse($provider->pivot->end_date)->format('m/d/Y') : 'Present' }}</small></td>
                                </tr>
                            @empty
                                <tr>
                        ...
                        <tbody>
                            @forelse($practice->documents as $document)
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        @if ($version)
                                            <a href="{{ asset('storage/' . $version->file_path) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                            ...
                            @empty
                                @if ($practice->document_path)
                                    <tr>
                                        <td class="fw-semibold">{{ $practice->document_original_name ?: 'Practice document' }}</td>
                                        <td>—</td>
                                        <td>—</td>
                                        <td>—</td>
                                        <td><span class="badge bg-label-success">Active</span></td>
                                        <td class="text-end">
                                            <a href="{{ asset('storage/' . $practice->document_path) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                                                <i class="ti tabler-download"></i>
                                            </a>
                                        </td>
                                    </tr>
                                @else
                                    <tr>
                                        <td colspan="6" class="text-center text-muted py-4">No documents on file for this practice.</td>
                                    </tr>
                                @endif
                            @endforelse
                        </tbody>
