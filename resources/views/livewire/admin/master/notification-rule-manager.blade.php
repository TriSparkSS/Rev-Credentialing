            ...
            <div class="row align-items-center g-3">
                ...
                <div class="col-lg-6">
                    <div class="input-group">
                        ...
                        <input type="search" wire:model.live="search" class="form-control border-start-0" placeholder="Search notification rules...">
                    </div>
                </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="6" class="text-center py-5 text-muted">No notification rules found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ $modalMode === 'create' ? 'Create Notification Rule' : 'Edit Notification Rule' }}</h5>
                        ...
                        <div class="modal-body">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" wire:model="formData.event_key" class="form-control @error('formData.event_key') is-invalid @enderror" placeholder="e.g. case.status_changed">
                                    @error('formData.event_key')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Channel</label>
                                    <select wire:model="formData.channel" class="form-select @error('formData.channel') is-invalid @enderror">
                                        @foreach ($channels as $key => $label)
                                            <option value="{{ $key }}">{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('formData.channel')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-12">
                                        ...
                                        @foreach ($adminRoles as $role)
                                            <div class="col-md-6">
                                                <div class="form-check">
                                                    <input type="checkbox" class="form-check-input" id="role_{{ $role->value }}" value="{{ $role->value }}" wire:model="formData.recipient_roles">
                                ...
                                <div class="col-12">
                                    <div class="form-check form-switch">
                                        <input type="checkbox" class="form-check-input" id="notificationRuleActive" wire:model="formData.is_active">
