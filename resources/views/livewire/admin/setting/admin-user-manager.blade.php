            ...
            <div class="row align-items-center g-3">
                ...
                <div class="col-lg-6">
                    <div class="input-group">
                        ...
                        <input type="search" wire:model.live="search" class="form-control border-start-0" placeholder="Search by name, email, or username...">
                    </div>
                </div>
                ...
                <tbody>
                    @forelse($records as $record)
                        <tr>
                            ...
                            <td>{{ $record->email }}</td>
                            <td>
                                <span class="badge {{ ($record->status ?? 'active') === 'active' ? 'bg-label-success' : 'bg-label-secondary' }}">
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-1">
                                    <button wire:click="openEditModal({{ $record->id }})" class="btn btn-sm btn-outline-secondary" title="Edit">
                                        <i class="ti tabler-edit"></i>
                                    </button>
                                    <button wire:click="openRoleModal({{ $record->id }})" class="btn btn-sm btn-outline-primary">
                    ...
                    @empty
                        <tr><td colspan="6" class="text-center py-5 text-muted">No admin users found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <form wire:submit.prevent="saveUser">
                        <div class="modal-body">
                            ...
                            @if (!$editingUserId)
                                <div class="mb-3">
                                    ...
                                    <input type="text" wire:model="userUsername" class="form-control @error('userUsername') is-invalid @enderror">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="email" wire:model="userEmail" class="form-control @error('userEmail') is-invalid @enderror">
                                @error('userEmail')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="mb-3">
                                <label class="form-label">
                                    Password @if (!$editingUserId)<span class="text-danger">*</span>@else<small class="text-muted">(leave blank to keep current)</small>@endif
                                </label>
                                <input type="password" wire:model="userPassword" class="form-control @error('userPassword') is-invalid @enderror" autocomplete="new-password">
                                @error('userPassword')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Status</label>
                                <select wire:model="userStatus" class="form-select @error('userStatus') is-invalid @enderror">
                            ...
                            @if (!$editingUserId)
                                <div class="mb-0">
                                    ...
                                    <select wire:model="userRole" class="form-select @error('userRole') is-invalid @enderror">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary">{{ $editingUserId ? 'Save Changes' : 'Create User' }}</button>
                        </div>
                    </form>
                </div>
            </div>
                            ...
                            <div class="d-flex flex-column gap-2">
                                @foreach ($availableRoles as $role)
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="admin_role_{{ $role['value'] }}" value="{{ $role['value'] }}" wire:model="selectedRoles">
                                        <label class="form-check-label" for="admin_role_{{ $role['value'] }}">{{ $role['label'] }}</label>
                                    </div>
                                @endforeach
                            </div>
